...
object BenchmarkingNode {
  ...
  def selectBestPlacementAlgorithm(constraints: List[NetworkChurnRate], requirements: List[Requirement]): PlacementAlgorithm = {
    ...
    for (requirement <- requirements) {
      requirement match {
        case l: LatencyRequirement => filterAlgos(l.name)
        case l: LoadRequirement => filterAlgos(l.name)
        case m: MessageHopsRequirement => filterAlgos(m.name)
  ...
  def getAlgorithmByName(algorithmName: String): PlacementAlgorithm = {
    if (!initialized) {
      parseConfiguration()
    }

    val algos = algorithms.toList.filter(p => p.placement.strategyName() == algorithmName)
    if (algos.nonEmpty) {
      algos.head
    } else {
      algorithms.head
