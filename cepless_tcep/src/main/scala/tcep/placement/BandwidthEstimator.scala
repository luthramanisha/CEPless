...
class BandwidthEstimator() extends ClusterActor {
  ...
  private val measurementNodes: ListBuffer[DataRateMeasurement] = new ListBuffer[DataRateMeasurement]()
  ...
  override def receive: Receive = {
    ...
    case BandwidthMeasurementSlotRequest() => // sender requests a measurement slot so that only the sender and no other node starts iperf to this node (at the same time)
      ...
      if(measurementNodes.exists(dm => dm.node.address.equals(senderMember.address))) {
    ...
    case AllBandwidthsRequest() =>
      val s = sender()
      val bandwidthMeasurements = if(this.measurementNodes.nonEmpty) AllBandwidthsResponse(this.measurementNodes.map(dm => dm.node -> dm.dataRate).toMap)
      else AllBandwidthsResponse(cluster.state.members.filterNot(_.hasRole("VivaldiRef")).map(m => m -> defaultRate).toMap)
      s ! bandwidthMeasurements
  ...
  /**
    ...
    * allow only one measurent (send to a remote node) at a time to prevent wrong values
    * currently inbound (iperf server being used) and outbound (contacting remote iperf server) can NOT happen at the same time
    */
  ...
  def addMeasurement(measurement: DataRateMeasurement): Unit = {
    val measurementEntry = measurementNodes.find(_.node == measurement.node)
    if(measurementEntry.isEmpty)
      measurementNodes += measurement
    else if(measurementEntry.get.dataRate != defaultRate) measurementNodes += measurement
  }

  def addMeasurementNode(addNode: Member): Boolean = {
    val dataRateMeasurement = measurementNodes.find(measurement => {
    ...
    measurementNodes.append(DataRateMeasurement(addNode))
  ...
  def removeMeasurementNode(removeNode: Member): Unit = {
    ...
    measurementNodes -= dataRateMeasurement.get
  ...
  private def updateMeasurements: Unit = {
    measurementNodes.foreach(measurementNode => {
  ...
  private def measure(node: Member): Future[Double] = {
    ...
    /*val measurement = Future {
      if (node.equals(cluster.selfMember))
        ...
        try {
          ...
          case e: Throwable => {
            log.warn(s"Iperf failed for $node, using default value. \n ${e.toString}")
            defaultRate
          }
        }
      }
    ...
    measurement.onComplete {
      ...
      case Failure(exception) => //log.warn(s"failed to measure bandwidth to $node, cause: \n $exception")
    }
    measurement*/
  }
