...
class GlobalOptimalBDPMultiJvmNode1 extends      GlobalOptimalBDPhMultiNodeTestSpec
class GlobalOptimalBDPMultiJvmNode2 extends      GlobalOptimalBDPhMultiNodeTestSpec
class GlobalOptimalBDPMultiJvmClient extends     GlobalOptimalBDPhMultiNodeTestSpec
class GlobalOptimalBDPMultiJvmPublisher1 extends GlobalOptimalBDPhMultiNodeTestSpec
class GlobalOptimalBDPMultiJvmPublisher2 extends GlobalOptimalBDPhMultiNodeTestSpec

abstract class GlobalOptimalBDPhMultiNodeTestSpec extends MultiJVMTestSetup {
  ...
  private def initUUT(cluster: Cluster): GlobalOptimalBDPAlgorithm.type = {
    ...
    uut.initialize(cluster, publisherNameMap = Some(publishers))
  ...
  "GlobalOptimalBDPAlgorithm" must {
    "select the node with minimum BDP sum to publishers and subscriber" in {
      ...
      runOn(client) {
        ...
        val placement: Map[Query, Member] = operators.map(op => op -> Await.result(uut.applyGlobalOptimalBDPAlgorithm(cluster, op, dependencies), uut.requestTimeout).member).toMap
