...
//  case class TumblingInstances (instances: Int) extends Window
  ...
  sealed trait CustomQuery extends UnaryQuery { val operatorName: String }
  sealed trait BenchmarkQuery extends UnaryQuery
  sealed trait KMeansQuery extends UnaryQuery
  sealed trait ForwardQuery extends UnaryQuery
  ...
  case class Test[A]                    (operatorName: String, sq: Query1[A], requirements: Set[Requirement]) extends Query1[A] with CustomQuery
  case class Benchmark[A]               (sq: Query1[A], requirements: Set[Requirement]) extends Query1[A] with BenchmarkQuery
  case class KMeans[A]                  (sq: Query1[A], requirements: Set[Requirement]) extends Query1[A] with KMeansQuery
  case class Forward[A]               (sq: Query1[A], requirements: Set[Requirement]) extends Query1[A] with ForwardQuery
