...
case class JoinNode(mode: Mode,
   ...
   Future{
     ...
     val updateListener: UpdateListener = (newEventBeans: Array[EventBean], _) => newEventBeans.foreach(eventBean => {
       ...
       */
       ...
       log.debug(s"creating new event $event with merged monitoringData: ${event.monitoringData.toList}")
       emitEvent(event)
     })
  ...
  def createDuplicateNode(hostInfo: HostInfo): ActorRef = {
    ...
    val res = NodeFactory.createJoinNode(mode, hostInfo, backupMode, mainNode, query, parentNode1, parentNode2, createdCallback, eventCallback, context)
    log.info(s"Spent ${System.currentTimeMillis() - startTime} milliseconds to initialize JoinNode")
    res
  }
