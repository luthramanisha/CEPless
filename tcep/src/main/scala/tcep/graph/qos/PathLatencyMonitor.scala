...
import tcep.data.Events.{Event, Latency, StartTime}
...
case class PathLatencyMonitor(query: Query, recordLatency: Option[LatencyMeasurement]) extends Monitor {
  ...
  override def onEventEmit(event: Event, status: Int): X = {
     ...
     */
    val accumulatedLatency = Duration.ofMillis(event.getMonitoringItem[Latency]().get.latency)
    val startTime: Long = event.getMonitoringItem[StartTime]().get.time
